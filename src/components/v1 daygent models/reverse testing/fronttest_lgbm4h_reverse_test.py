...
try:
    ...
    import subprocess
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'numpy', 'pandas', 'scikit-learn', 'tqdm', 'joblib', 'pyarrow', 'lightgbm', 'matplotlib'])
    import numpy as np
    import pandas as pd
    from tqdm import tqdm
    from sklearn.preprocessing import StandardScaler
...
OUTPUT_DIR = os.path.join(BASE_DIR, 'lgbm_4h_reverse_fronttest')
os.makedirs(OUTPUT_DIR, exist_ok=True)

print("="*80)
print("🔄 LGBM 4H Reverse Test on Fronttest SPY 4H")
...
config_path = first_existing(MODEL_DIR, ['deployment_config.json', 'deployment_config_4h.json'])
results_path = first_existing(MODEL_DIR, ['results_4h_only.json', 'results_lgbm_4h.json'])
model_path = first_existing(MODEL_DIR, ['lightgbm_financial_4h_only.joblib', 'lgbm_4h.joblib'])
scaler_path = first_existing(MODEL_DIR, ['scaler_4h_only.joblib', 'scaler_4h.joblib'])
...
# %% [cell] STEP 4: FEATURE EXTRACTION HELPERS (MATCH 4h CONTRACT)
print("\n🔧 Preparing feature helpers (4h 16-feature contract)...")
...
proba = lgbm_model.predict_proba(X_test_scaled)[:, 1]
...
summary_path = os.path.join(OUTPUT_DIR, 'fronttest_summary_lgbm4h.json')
preds_csv = os.path.join(OUTPUT_DIR, 'fronttest_predictions_lgbm4h.csv')
daily_csv = os.path.join(OUTPUT_DIR, 'fronttest_daily_metrics_lgbm4h.csv')
...
try:
    ...
    if len(daily):
        ...
        ax.set_title('Daily Accuracy (4h LGBM)', fontsize=13)
        ...
        out_plot = os.path.join(OUTPUT_DIR, 'fronttest_daily_accuracy_lgbm4h.png')
