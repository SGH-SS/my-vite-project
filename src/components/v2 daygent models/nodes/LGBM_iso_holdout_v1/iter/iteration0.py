...
# Run directory using iterationN naming (iteration0, iteration1, ...)
RUNS_DIR = os.path.join(ARTIFACT_DIR, 'runs')
os.makedirs(RUNS_DIR, exist_ok=True)
RUN_TS = datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')

# Determine next iteration index based on existing folders
existing_iterations = []
try:
    for entry_name in os.listdir(RUNS_DIR):
        entry_path = os.path.join(RUNS_DIR, entry_name)
        if os.path.isdir(entry_path) and entry_name.startswith('iteration'):
            suffix = entry_name[len('iteration'):]
            if suffix.isdigit():
                existing_iterations.append(int(suffix))
except FileNotFoundError:
    pass

next_iter_idx = (max(existing_iterations) + 1) if existing_iterations else 0

# Create the next available iteration directory atomically to avoid collisions
while True:
    candidate_dir = os.path.join(RUNS_DIR, f'iteration{next_iter_idx}')
    try:
...
if len(X_tr) and len(y_tr) and len(X_va):
    model.fit(
        X_tr,
        y_tr,
        eval_set=[(X_va, y_va)],
...
if len(val_proba):
    for thr in np.round(np.arange(0.31, 0.801, 0.01), 2):
        preds_thr = (val_proba >= thr).astype(int)
        acc_thr = accuracy_score(y_va, preds_thr)
        # Select highest threshold in case of tie
        if acc_thr > best_val_acc or (acc_thr == best_val_acc and thr > best_thr):
            best_val_acc = acc_thr
            best_thr = float(thr)

# Refit on full training using best number of trees if available
n_estimators_final = lgb_params["n_estimators"]
if hasattr(model, "best_iteration_") and model.best_iteration_:
    try:
        n_estimators_final = int(model.best_iteration_)
    except Exception:
        n_estimators_final = lgb_params["n_estimators"]

full_params = dict(lgb_params)
full_params["n_estimators"] = n_estimators_final
model_full = lgb.LGBMClassifier(**full_params)
...
results = {
    ...
    "validation_window": val_acc_window,
    "test_window": test_acc_window,
}
